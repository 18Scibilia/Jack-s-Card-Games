...
bool SolitaireDraw::setxychar(int x, int y, const char* pBuf, int len, WORD ColorAttrib)
{
...
bool SolitaireDraw::setxychar(int x, int y, char Buf, WORD ColorAttrib)
{
	return(setxychar(x, y, &Buf, 1));
}

void SolitaireDraw::gotoxy(int column, int line)
{
	COORD coord;
	coord.X = column;
	coord.Y = line;
